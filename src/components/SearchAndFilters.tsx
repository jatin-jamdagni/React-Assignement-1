...
import { setSearchQuery, setFilter, setCategoryFilter } from '@/store/tasksSlice';
import type { Category, FilterStatus } from '@/store/types';
...
const SearchAndFilters = () => {
  ...
  const { searchQuery, filter, categoryFilter } = useAppSelector(s => s.tasks);
  ...
  return (
    <div className="space-y-3">
      <div className="relative">
        ...
        />
        {searchQuery && (
          <button onClick={() => dispatch(setSearchQuery(''))} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground">
            <X className="h-4 w-4" />
          </button>
        )}
      </div>
